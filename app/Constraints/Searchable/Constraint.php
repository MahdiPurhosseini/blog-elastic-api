...
namespace WM\Core\Constraints\Searchable;
...
class Constraint
{
    ...
    protected function doApply(Builder $builder, $field, $mode = Constraint::MODE_AND)
    {
        if ($this->operator == Constraint::OPERATOR_IN) {
            ...
            $builder->$method($field, $this->value);
        } elseif ($this->operator == Constraint::OPERATOR_BETWEEN) {
            $method = $mode != static::MODE_OR ? 'whereBetween' : 'orWhereBetween';
            $builder->$method($field, $this->value);
        } elseif ($this->operator == Constraint::OPERATOR_NOT_BETWEEN) {
            $method = $mode != static::MODE_OR ? 'whereNotBetween' : 'orWhereNotBetween';
            $builder->$method($field, $this->value);
    ...
    protected static function parseOperatorAndValue($value, $is_negation)
    {
        ...
        if ($result = static::parseBetweenOperator($value, $is_negation)) {
            return $result;
        }
    ...
    {
        if (preg_match('/^\((g|ge|l|le)\)(.+)$/', $value, $match)) {
            $operator = null;
            switch ($match[1]) {
                case 'g':
     ...
     * @return string[]|false
     */
    protected static function parseBetweenOperator($value, $is_negation)
    {
        if (preg_match('/^<.+,.+>$/', $value)) {
            $value = preg_split('/,/', trim($value, "<>"));
            return [$is_negation ? static::OPERATOR_NOT_BETWEEN : static::OPERATOR_BETWEEN, $value];
        }
        return false;
    }
    /**
